...
class HE_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        sur_name = os.path.join(self.files_list.iloc[idx, 0])
        dec_name = os.path.join(self.files_list.iloc[idx, 1])

        sur_image = io.imread(sur_name)
        dec_image= io.imread(dec_name)
        
        sample = {'input_sur': sur_image, 'input_dec': dec_image}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        input, output = sample['input_sur'], sample['input_dec']
        ...
        return {'input_sur': torch.from_numpy(input),
                'input_dec': torch.from_numpy(output)}
...
def show_patch(dataloader, index = 3):
    for i_batch, sample_batched in enumerate(dataloader):
        print(i_batch, sample_batched['input_sur'].size(), 
              sample_batched['input_dec'].size())

        # observe 4th batch and stop.
        if i_batch == index:
            plt.figure()
            input_batch, label_batch = sample_batched['input_sur'], sample_batched['input_dec']
